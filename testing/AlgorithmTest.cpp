...
TEST_F(AlgorithmTest, IdentifyBipartite) {
    for(uint32_t i = 0; i < 200; ++i) {
        ...
        for (int u : output.first) {
            EXPECT_EQ(output.second.find(u), output.second.end()); // each vertex is in exactly one set
            for (int v : output.first)
                if (u != v)
                    EXPECT_FALSE(input.has_edge(u, v)); // each half is an independent set
        }

        // Repeat with second set; don't need to check against first set since we would have found it already
...
TEST_F(AlgorithmTest, MaxBipartiteMatch){
    for(uint32_t i = 0; i < 200; ++i) {
        ...
        std::list<std::pair<int, int>> result = graph_alg::maximum_bipartite_matching(input, -1, -2);
