...
// endomorphic)
template<typename T> std::unordered_set<T> subset_automorphism(const std::unordered_map<T, T>& f);
template<typename T> std::unordered_set<T> subset_automorphism(const std::map<T, T>& f);
}; // namespace induction
