...
template<typename T, typename Compare>
basic_binary_search_tree<T, Compare>::basic_binary_search_tree(const Compare& comp,
                                                               bool duplicates) :
...
template<typename T, typename Compare>
typename basic_binary_search_tree<T, Compare>::iterator
  basic_binary_search_tree<T, Compare>::erase(typename binary_tree<T>::iterator it) {
    ...
    // and delete
    auto swap_out = [&parent, &node, this, is_left_child,
                     is_root](typename binary_tree<T>::node* ptr) {
        if (ptr != nullptr) {
            if (ptr->parent == node) {
                if (ptr == node->right) {
