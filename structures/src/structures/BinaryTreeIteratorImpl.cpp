...
template <typename T> const T& TreeIteratorImpl<T>::operator*() const noexcept
{
    return current->_item;
}

template <typename T> T& TreeIteratorImpl<T>::operator*() noexcept
{
...
template <typename T>
typename BinaryTree<T>::BinaryNode* PreOrderTreeIteratorImpl<T>::preOrderEnd(
    typename BinaryTree<T>::BinaryNode* root) noexcept
{
    if (!root)
        return root;

    for (typename BinaryTree<T>::BinaryNode *left(root->_left), *right(root->_right); left || right;
...
template <typename T>
typename BinaryTree<T>::BinaryNode* InOrderTreeIteratorImpl<T>::leftmostDescendant(
    typename BinaryTree<T>::BinaryNode* root) noexcept
{
    if (!root)
        return root;
...
template <typename T>
typename BinaryTree<T>::BinaryNode* InOrderTreeIteratorImpl<T>::rightmostDescendant(
    typename BinaryTree<T>::BinaryNode* root) noexcept
{
    if (!root)
        return root;
...
template <typename T>
typename BinaryTree<T>::BinaryNode* PostOrderTreeIteratorImpl<T>::postOrderStart(
    typename BinaryTree<T>::BinaryNode* root) noexcept
{
    if (!root)
        return root;
...
template <typename T>
TreeIteratorImpl<T>::TreeIteratorImpl(typename BinaryTree<T>::BinaryNode* current)
    : current(current)
{
}
...
{
    typename BinaryTree<T>::BinaryNode* parent = this->current->_parent;
    this->current = (parent && (parent->_right == this->current) && parent->_left)
        ? preOrderEnd(parent->_left)
        : parent;
...
template <typename T> InOrderTreeIteratorImpl<T>& InOrderTreeIteratorImpl<T>::operator++() noexcept
{
...
{
    typename BinaryTree<T>::BinaryNode* parent = this->current->_parent;
    this->current = (parent && (parent->_left == this->current) && parent->_right)
        ? postOrderStart(parent->_right)
        : parent;
