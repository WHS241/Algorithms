...
namespace tree {
    ...
    template<typename T>
    typename binary_tree<T>::iterator binary_tree<T>::begin(traversal traversal) {
    ...
    template<typename T>
    uint32_t binary_tree<T>::size() const noexcept { return _size; }

    template<typename T>
    binary_tree<T>::node::node(
            const T &item, node *parent, node *left, node *right)
    ...
    template<typename T>
    binary_tree<T>::node::~node() noexcept {
        delete left;
        delete right;
    }

    template<typename T>
    binary_tree<T>::node::node(node &&src) noexcept {
        *this = std::move(src);
    }
    ...
    template<typename T>
    typename binary_tree<T>::node *binary_tree<T>::node::clone() const {
    ...
    template<typename T>
    typename binary_tree<T>::node *binary_tree<T>::_get_node(
