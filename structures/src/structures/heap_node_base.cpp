...
namespace heap {
    ...
    const T &node_base<T, Compare>::node::operator*() const {
        return _value;
    }

    template<typename T, typename Compare>
    node_base<T, Compare>::node::~node() noexcept {
        for (node *child : _children)
            delete child;
    }

    template<typename T, typename Compare>
    T &node_base<T, Compare>::node::operator*() {
    ...
    template<typename T, typename Compare>
    void node_base<T, Compare>::insert(const T &item) {
        this->add(item);
    }

    template<typename T, typename Compare>
    uint32_t node_base<T, Compare>::size() const noexcept {
        return _size;
    }
