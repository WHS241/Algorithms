...
namespace tree {
    ...
    template<typename T, typename Compare>
    void AVL_tree<T, Compare>::insert(const T &item) {
        ...
        t_node *next
                = go_left ? dynamic_cast<t_node *>(ptr->left) : dynamic_cast<t_node *>(ptr->right);
        while (next != nullptr) {
            ptr = next;
    ...
    template<typename T, typename Compare>
    void AVL_tree<T, Compare>::balance_tree(
            typename AVL_tree<T, Compare>::t_node *start) noexcept {
        enum rotate {
            left, right, none
        };
    ...
    template<typename T, typename C>
    void AVL_tree<T, C>::remove(typename binary_tree<T>::iterator it) {
