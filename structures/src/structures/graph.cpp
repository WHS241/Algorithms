...
namespace graph {
    ...
    graph<T, Directed, Weighted>::graph(const graph<T, Directed, Weighted> &src)
            : _type(src._type), _translation(src._translation), _reverse_translation(src._reverse_translation) {
        _set_type(_type, src._impl.get());
    }

    template<typename T, bool Directed, bool Weighted>
    const graph<T, Directed, Weighted> &
    graph<T, Directed, Weighted>::operator=(const graph<T, Directed, Weighted> &rhs) {
        if (this != &rhs) {
    ...
    template<typename T, bool Directed, bool Weighted>
    const graph<T, Directed, Weighted> &
    graph<T, Directed, Weighted>::operator=(graph<T, Directed, Weighted> &&rhs) noexcept {
        if (this != &rhs) {
            _impl.reset();
    ...
    template<typename T, bool Directed, bool Weighted>
    uint32_t graph<T, Directed, Weighted>::order() const noexcept { return _impl->order(); }

    template<typename T, bool Directed, bool Weighted>
    bool graph<T, Directed, Weighted>::has_edge(const T &start, const T &dest) const noexcept {
        if (_translation.find(start) == _translation.end() || _translation.find(dest) == _translation.end())
            return false;
    ...
    template<typename T, bool Directed, bool Weighted>
    std::vector<T> graph<T, Directed, Weighted>::vertices() const { return _reverse_translation; }
