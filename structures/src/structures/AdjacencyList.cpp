...
uint32_t AdjacencyList::order() const noexcept { return graph.size(); }
...
double AdjacencyList::edgeCost(const uint32_t& start, const uint32_t& dest) const noexcept
{
    ...
    auto it = std::find_if(graph[start].begin(), graph[start].end(),
...
std::list<uint32_t> AdjacencyList::neighbors(const uint32_t& start) const
{
    ...
    std::transform(graph[start].begin(), graph[start].end(), std::back_inserter(result),
...
void AdjacencyList::removeEdge(const uint32_t& start, const uint32_t& dest)
{
    ...
    if (it != graph[start].end()) {
        ...
        if (!dir) {
            it = std::find_if(graph[dest].begin(), graph[dest].end(),
...
void AdjacencyList::isolate(const uint32_t& target)
{
    ...
    if (!dir) {
        ...
        for (auto& e : edges)
            graph[e.first].erase(std::find_if(graph[e.first].begin(), graph[e.first].end(),
...
void AdjacencyList::clear() noexcept { graph.clear(); }