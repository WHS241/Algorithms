...
template <typename T>
dynamic_matrix<T>::row_wrapper::row_wrapper(dynamic_matrix<T>* m, std::size_t index) : _matrix(m), _index(index) {}

template <typename T>
typename dynamic_matrix<T>::const_reference dynamic_matrix<T>::const_row_wrapper::operator[](std::size_t index) const {
    std::size_t pos = _index * _matrix->_cols + index;
    return _matrix->_is_initialized(pos) ? _matrix->_elements[pos] : _matrix->_default_value;
}

template <typename T>
typename dynamic_matrix<T>::const_reference dynamic_matrix<T>::const_row_wrapper::at(std::size_t index) const {
    _matrix->_check_col(index);
    std::size_t pos = _index * _matrix->_cols + index;
    return _matrix->_is_initialized(pos) ? _matrix->_elements[pos] : _matrix->_default_value;
}
...
template <typename T>
dynamic_matrix<T>::const_row_wrapper::const_row_wrapper(const dynamic_matrix<T>* m, std::size_t index) : _matrix(m), _index(index) {}
...
template <typename T>
dynamic_matrix<T>::dynamic_matrix(std::size_t m, std::size_t n, const T& default_value): _allocator(), _check_allocator(), _check_size(0U), _default_value(default_value), _rows(m), _cols(n) {
    // out of bounds error
    std::size_t size = m * n;
...
template<typename T>
template <typename, typename>
dynamic_matrix<T>::dynamic_matrix(const std::initializer_list<std::initializer_list<T>>& src) : dynamic_matrix<T>(src.size(), std::max(src, [](auto x, auto y) {return x.size() < y.size();}).size()) {
...
template <typename T>
dynamic_matrix<T>::~dynamic_matrix() noexcept {
...
template <typename T>
dynamic_matrix<T>::dynamic_matrix(const dynamic_matrix<T>& src): _allocator(), _check_allocator(), _check_size(0U), _default_value(src._default_value), _rows(src._rows), _cols(src._cols) {
...
template <typename T>
dynamic_matrix<T>& dynamic_matrix<T>::operator=(const dynamic_matrix<T>& rhs) {
    if (this != &rhs) {
...
template <typename T>
dynamic_matrix<T>& dynamic_matrix<T>::operator=(dynamic_matrix<T>&& rhs) {
    if (this != &rhs) {
        this->~dynamic_matrix();
...
template <typename T>
std::size_t dynamic_matrix<T>::num_rows() const {
    return _rows;
}

template <typename T>
std::size_t dynamic_matrix<T>::num_cols() const {
    return _cols;
}

template <typename T>
dynamic_matrix<T> dynamic_matrix<T>::subarray(std::size_t rows, std::size_t cols, std::size_t first_row, std::size_t first_col) const {
    return subarray(rows, cols, first_row, first_col, _default_value); 
}

template <typename T>
dynamic_matrix<T> dynamic_matrix<T>::subarray(std::size_t rows, std::size_t cols, std::size_t first_row, std::size_t first_col, const T& pad_value) const {
...
template <typename T>
void dynamic_matrix<T>::resize(std::size_t rows, std::size_t cols, std::size_t first_row, std::size_t first_col) {
    *this = subarray(rows, cols, first_row, first_col);
}

template <typename T>
template <typename U>
dynamic_matrix<std::invoke_result_t<std::plus<>, T, U>> dynamic_matrix<T>::operator+(const dynamic_matrix<U>& rhs) const {
...
dynamic_matrix<T>& dynamic_matrix<T>::operator+=(const dynamic_matrix<U>& rhs) noexcept(std::is_nothrow_invocable_r_v<T, std::plus<>, T, U>) {
    if (_rows != rhs._rows || _cols != rhs._cols)
        throw std::invalid_argument("Dimension mismatch");
...
template <typename T>
template <typename U>
dynamic_matrix<std::invoke_result_t<std::minus<>, T, U>> dynamic_matrix<T>::operator-(const dynamic_matrix<U>& rhs) const {
    if (_rows != rhs._rows || _cols != rhs._cols)
        throw std::invalid_argument("Dimension mismatch");
...
template <typename T>
template <typename U, typename, typename>
dynamic_matrix<T>& dynamic_matrix<T>::operator-=(const dynamic_matrix<U>& rhs) noexcept(std::is_nothrow_invocable_r_v<T, std::minus<>, T, U>) {
    if (_rows != rhs._rows || _cols != rhs._cols)
        throw std::invalid_argument("Dimension mismatch");
...
template <typename T>
dynamic_matrix<T> dynamic_matrix<T>::operator*(const T& rhs) const {
...
template <typename T>
dynamic_matrix<T>& dynamic_matrix<T>::operator*=(const T& rhs) noexcept(std::is_nothrow_invocable_r_v<T, std::multiplies<>, T, T>) {
    if constexpr (std::is_nothrow_invocable_r_v<T, std::multiplies<>, T, T>) {
...
template <typename T>
dynamic_matrix<T>& dynamic_matrix<T>::operator/=(const T& rhs) noexcept(std::is_nothrow_invocable_r_v<T, std::divides<>, T, T>) {
    if constexpr (std::is_nothrow_invocable_r_v<T, std::divides<>, T, T>) {
        _default_value = _default_value / rhs;
        for(std::size_t i = 0; i < _check_size; ++i)
            _elements[_check_2[i]] = _elements[_check_2[i]] / _default_value;
...
template <typename T>
template <typename U>
dynamic_matrix<std::invoke_result_t<std::multiplies<>, T, U>> dynamic_matrix<T>::operator*(const dynamic_matrix<U>& rhs) const {
    if (_cols != rhs._rows)
        throw std::invalid_argument("Dimension mismatch");
    ...
    if (dimensions[1] >= 2 * dimensions[0] || (dimensions[1] == dimensions[0] && dimensions[2] >= 2 * dimensions[0])) {
        // Case 1: skinny LHS
        ...
        if (dimensions[0] == _cols) {
            ...
            for (std::size_t i = 0; i < num_large_rows; ++i) {
                for (std::size_t j = 0; j < num_large_cols; ++j) {
                    ...
                    std::size_t row_bound(std::min(_cols, _rows - i * _cols)), col_bound(std::min(_cols, rhs._cols - j * _cols));
        ...
        if (dimensions[0] == rhs._cols) {
            ...
            for (std::size_t i = 0; i < num_large_rows; ++i)
                for (std::size_t j = 0; j < num_large_cols; ++j)\
                    lhs_sub[i][j] = this->subarray(rhs._cols, rhs._cols, i * rhs._cols, j * rhs._cols, T());
    ...
    std::size_t row_split(_rows / 2 + _rows % 2), mid_split(_cols / 2 + _cols % 2), col_split(rhs._cols / 2 + rhs._cols % 2);
...
template <typename T>
bool dynamic_matrix<T>::operator==(const dynamic_matrix<T>& rhs) const {
    if(!(_rows == rhs._rows && _cols == rhs._cols))
        return false;
    for (std::size_t i = 0; i < _rows; ++i)
        for (std::size_t j = 0; j < _cols; ++j)
...
template <typename T>
void dynamic_matrix<T>::_check_row(std::size_t pos) const {
    if (pos >= _rows)
        throw std::out_of_range(std::to_string(pos));
}

template <typename T>
void dynamic_matrix<T>::_check_col(std::size_t pos) const {
    if (pos >= _cols)
        throw std::out_of_range(std::to_string(pos));
}

template <typename T>
bool dynamic_matrix<T>::_is_initialized(std::size_t pos) const noexcept {
    return _check_1[pos] < _check_size && _check_2[_check_1[pos]] == pos;
}

template <typename T>
void dynamic_matrix<T>::_initialize(std::size_t pos, const T& value) {
