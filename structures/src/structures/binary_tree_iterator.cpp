...
template <typename T> T& tree_iterator<T>::operator*() { return **_impl; }

template <typename T> const T* tree_iterator<T>::operator->() const { return &operator*(); }

template <typename T> T* tree_iterator<T>::operator->() { return &operator*(); }
...
template <typename T> tree_const_iterator<T>::tree_const_iterator(const tree_const_iterator<T>& src)
{
    tree_iterator_impl<T>* ptr = src._impl.get();
    auto prePtr = dynamic_cast<pre_order_iterator_impl<T>*>(ptr);
    if (prePtr) {
...
template <typename T> const T& tree_const_iterator<T>::operator*() const { return **_impl; }

template <typename T> const T* tree_const_iterator<T>::operator->() const { return &operator*(); }
