...
template<bool Directed, bool Weighted, typename EdgeWeight>
bool adjacency_list<Directed, Weighted, EdgeWeight>::has_edge(const uint32_t& start,
...
template<bool Directed, bool Weighted, typename EdgeWeight>
EdgeWeight adjacency_list<Directed, Weighted, EdgeWeight>::edge_cost(const uint32_t& start,
...
template<bool Directed, bool Weighted, typename EdgeWeight>
std::list<uint32_t>
  adjacency_list<Directed, Weighted, EdgeWeight>::neighbors(const uint32_t& start) const {
...
std::list<std::pair<uint32_t, EdgeType>>
  adjacency_list<Directed, Weighted, EdgeType>::edges(const uint32_t& start) const {
            ...
            for (const _t_edge& edge : _graph[i])
                if (selected[edge.first])
                    if constexpr (Directed)
                        subgraph->force_add(translate_to_sub[i], translate_to_sub[edge.first],
                                            edge.second);
...
template<bool Directed, bool Weighted, typename EdgeWeight>
void adjacency_list<Directed, Weighted, EdgeWeight>::set_edge(const uint32_t& start,
                                                              const uint32_t& dest,
...
template<bool Directed, bool Weighted, typename EdgeWeight>
void adjacency_list<Directed, Weighted, EdgeWeight>::force_add(const uint32_t& start,
                                                               const uint32_t& dest,
                                                               const EdgeWeight& cost) {
    if constexpr (Directed) {
        _graph[start].emplace_back(dest, cost);
...
void adjacency_list<Directed, Weighted, EdgeWeight>::sanitize() {
    ...
    for (uint32_t i = 0; i < num_vert; ++i) {
        _graph[i].remove_if([&i, &num_vert, &counter, &tracker,
                             &array_size](const std::pair<uint32_t, EdgeWeight>& e) {
            uint32_t j = e.first;
            if (i == j)
                return true;
...
template<bool Directed, bool Weighted, typename EdgeWeight>
void adjacency_list<Directed, Weighted, EdgeWeight>::remove_edge(const uint32_t& start,
                                                                 const uint32_t& dest) {
    auto it = std::find_if(_graph[start].begin(), _graph[start].end(),
                           [&dest](const _t_edge& edge) { return edge.first == dest; });
    if (it != _graph[start].end()) {
...
void adjacency_list<Directed, Weighted, EdgeWeight>::isolate(const uint32_t& target) {
    if constexpr (!Directed) {
        ...
        for (const _t_edge& e : edges)
            _graph[e.first].erase(std::find_if(
              _graph[e.first].begin(), _graph[e.first].end(),
