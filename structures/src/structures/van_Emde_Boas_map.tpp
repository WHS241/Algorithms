...
template<typename T>
_vEB_map_iterator<T>::_vEB_map_iterator(van_Emde_Boas_map<T>* _tree, std::size_t index) :
    _tree(_tree), _curr_index(index) {}
...
template<typename T>
typename _vEB_map_const_iterator<T>::pointer
  _vEB_map_const_iterator<T>::operator->() const noexcept {
    return _tree->_elements + _curr_index;
}
...
template<typename T>
_vEB_map_const_iterator<T>::_vEB_map_const_iterator(const van_Emde_Boas_map<T>* _tree,
                                                    std::size_t index) :
    _tree(_tree), _curr_index(index) {}

template<typename T>
van_Emde_Boas_map<T>::van_Emde_Boas_map(std::size_t range) : _tree(range), _allocator() {
...
template<typename T>
typename van_Emde_Boas_map<T>::iterator van_Emde_Boas_map<T>::begin() noexcept {
...
template<typename T>
typename van_Emde_Boas_map<T>::const_reverse_iterator
  van_Emde_Boas_map<T>::crbegin() const noexcept {
...
template<typename T> bool van_Emde_Boas_map<T>::empty() const noexcept {
    return size() != 0;
}
...
template<typename T> void van_Emde_Boas_map<T>::clear() noexcept {
    for (std::size_t i = _tree.min();; i = _tree.find_next(i)) {
        std::destroy_at(_elements + i);
        if (i == _tree.max_size())
            break;
    }
    _tree.clear();
...
template<typename T>
std::pair<typename van_Emde_Boas_map<T>::iterator, bool>
  van_Emde_Boas_map<T>::insert(const typename van_Emde_Boas_map<T>::value_type& value) {
...
template<typename T>
std::pair<typename van_Emde_Boas_map<T>::iterator, bool>
  van_Emde_Boas_map<T>::insert(typename van_Emde_Boas_map<T>::value_type&& value) {
    bool new_insert = !_tree.contains(value.first);
    iterator ret_val(this, value.first);
    if (new_insert) {
        _tree.insert(value.first);
        try {
            std::construct_at(_elements + value.first, std::move(value));
        } catch (...) {
...
template<typename T>
typename van_Emde_Boas_map<T>::iterator
  van_Emde_Boas_map<T>::erase(typename van_Emde_Boas_map<T>::iterator pos) {
    if (pos == end() || pos._tree != this)
        throw std::invalid_argument("Invalid iterator: does not point into this tree");
    std::size_t index = pos._curr_index;
    if (!_tree.contains(index))
        throw std::invalid_argument("Invalid iterator: element does not exist");
    ++pos;
    _tree.erase(index);
    std::destroy_at(_elements + index);
    return pos;
}

template<typename T>
typename van_Emde_Boas_map<T>::iterator
  van_Emde_Boas_map<T>::erase(typename van_Emde_Boas_map<T>::const_iterator pos) {
...
template<typename T>
typename van_Emde_Boas_map<T>::iterator
  van_Emde_Boas_map<T>::erase(typename van_Emde_Boas_map<T>::iterator first,
                              typename van_Emde_Boas_map<T>::iterator last) {
    while (first != last) {
        first = erase(first);
    }
    return last;
...
template<typename T>
typename van_Emde_Boas_map<T>::iterator
  van_Emde_Boas_map<T>::erase(typename van_Emde_Boas_map<T>::const_iterator first,
...
template<typename T>
typename van_Emde_Boas_map<T>::mapped_type&
  van_Emde_Boas_map<T>::at(const typename van_Emde_Boas_map<T>::key_type& key) {
    return const_cast<mapped_type&>(static_cast<const van_Emde_Boas_map<T>*>(this)->at(key));
}

template<typename T>
const typename van_Emde_Boas_map<T>::mapped_type&
  van_Emde_Boas_map<T>::at(const typename van_Emde_Boas_map<T>::key_type& key) const {
    valid_check(key);
...
template<typename T>
typename van_Emde_Boas_map<T>::mapped_type&
  van_Emde_Boas_map<T>::operator[](const typename van_Emde_Boas_map<T>::key_type& key) {
    if (!_tree.contains(key))
...
template<typename T>
typename van_Emde_Boas_map<T>::iterator
  van_Emde_Boas_map<T>::find(const typename van_Emde_Boas_map<T>::key_type& key) {
    return _tree.contains(key) ? iterator(this, key) : end();
}

template<typename T>
typename van_Emde_Boas_map<T>::const_iterator
  van_Emde_Boas_map<T>::find(const typename van_Emde_Boas_map<T>::key_type& key) const {
