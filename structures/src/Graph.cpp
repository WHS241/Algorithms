...
template<typename T>
const Graph<T>& Graph<T>::operator=(const Graph<T>& rhs) {
    if (this != &rhs) {
        Graph<T> temp(rhs);
        *this = std::move(temp);
    }
...
template<typename T>
const Graph<T>& Graph<T>::operator=(Graph<T>&& rhs) {
    if (this != &rhs) {
        impl.reset();
...
template<typename T>
uint32_t Graph<T>::order() const noexcept {
    return impl->order();
}

template<typename T>
bool Graph<T>::hasEdge(const T& start, const T& dest) const noexcept {
...
template<typename T>
double Graph<T>::edgeCost(const T& start, const T& dest) const {
    return impl->edgeCost(translation.at(start), translation.at(dest));
}

template<typename T>
uint32_t Graph<T>::degree(const T& vertex) const {
    return impl->degree(translation.at(vertex));
}

template<typename T>
std::list<T> Graph<T>::neighbors(const T& start) const {
    auto representation = impl->neighbors(translation.at(start));
    std::list<T> result;
    std::transform(representation.begin(), representation.end(), std::back_inserter(result), [this](uint32_t& x) {
        return reverse_translation[x];
        });
    return result;
}

template<typename T>
std::list<std::pair<T, double>> Graph<T>::edges(const T& src) const {
    auto representation = impl->edges(src);
    std::list<std::pair<T, double>> result;
    std::transform(representation.begin(), representation.end(), std::back_inserter(result), [this](std::pair<uint32_t, double>& edge) {
...
template<typename T>
bool Graph<T>::directed() const noexcept {
    return impl->directed();
}

template<typename T>
bool Graph<T>::weighted() const noexcept {
    return impl->weighted();
}

template<typename T>
Graph<T> Graph<T>::convert(GraphType type) const {
    Graph<T> result(type);
...
template<typename T>
void Graph<T>::setEdge(const T& start, const T& dest, double cost) {
    if (start == dest)
...
template<typename T>
uint32_t Graph<T>::addVertex(const T& name) {
...
template<typename T>
void Graph<T>::removeEdge(const T& start, const T& dest) {
    impl->removeEdge(translation.at(start), translation.at(dest));
}

template<typename T>
void Graph<T>::isolate(const T& start) {
...
template<typename T>
void Graph<T>::setType(GraphType type, const GraphImpl* const src) {
...
template<typename T>
void Graph<T>::setEmpty(GraphType type, bool directed, bool weighted) {
    switch (type) {
    case AdjMatrix :
