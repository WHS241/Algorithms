...
public:
    ...
    typedef std::pair<const uint32_t, T> value_type;
...
private:
    _vEB_map_iterator(van_Emde_Boas_map<T>*, uint32_t);
...
private:
    _vEB_map_const_iterator(const van_Emde_Boas_map<T>*, uint32_t);
    friend class van_Emde_Boas_map<T>;
...
public:
    ...
    explicit van_Emde_Boas_map(uint32_t range = 0);
