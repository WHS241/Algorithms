...
public:
    AVLSearchTree(Compare comp = Compare())
        : BinarySearchTree<T, Compare>(comp) {};

    template <typename It>
    AVLSearchTree(It first, It last, Compare comp = Compare())
...
protected:
    struct AVLNode : virtual public BinaryTree<T>::BinaryNode {
        AVLNode(const T& item = T(), AVLNode* parent = nullptr, AVLNode* left = nullptr,
