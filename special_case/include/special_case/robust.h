...
std::unordered_map<T, bool, Hash, KeyEqual> CNF_2_SAT(
    ...
    std::unordered_map<T, std::unordered_set<clause_ptr, util::it_hash<clause_ptr>>, Hash, KeyEqual> clause_map(
        1, hash, key_eq);
    ...
    // 
    // Successful: Remove forced/set variables from clause_map, remove satisfied clauses from expr, return map containing all forced settings (including original, so never empty)
    // Unsuccessful: expr and clause_map untouched, return empty map
    //
    // If given a variable already set, returns a map with only the input setting if matches. If conflicts, returns unsuccessful
    auto propagate = [&expr, &clause_map, &result, &hash, &key_eq](const T& var, bool setting) -> std::unordered_map<T, bool, Hash, KeyEqual> {
        ...
        while (!to_set.empty() && !conflict) {
            ...
            if (subresult.find(next.first) != subresult.end()) {
                ...
                for (clause_ptr it : clause_map.at(next.first)) {
                    ...
                    std::pair<T, bool> found = *std::find_if(it->begin(), it->end(), [&next, &key_eq](const std::pair<T, bool>& x) {return key_eq(x.first, next.first);});
    ...
    for (const std::pair<T, bool>& forced : single_variables) {
        std::unordered_map<T, bool, Hash, KeyEqual> subresult = propagate(forced.first, forced.second);
        if (subresult.empty())
    ...
    while (!expr.empty()) {
        ...
        std::unordered_map<T, bool, Hash, KeyEqual> subresult = propagate(next_to_set.first, next_to_set.second);
