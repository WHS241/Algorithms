...
namespace graph_alg {
    ...
    std::pair<std::unordered_set<T>, std::unordered_set<T>> verify_bipartite(const graph::graph<T, false, Weighted>& input) {
        std::pair<std::unordered_set<T>, std::unordered_set<T>> result;

        depth_first_forest(input, input.vertices()[0], [&result, &input](const T& v) {
            bool in_second = (result.second.find(v) != result.second.end());
            if (!in_second)
                // either this vertex is in the first set or is a root. Either way, we can put it in first
                result.first.insert(v);
            
            // process all of the current's neighbors
            for (const T& w : input.neighbors(v)) {
                if ((in_second && result.second.find(w) != result.second.end()) || (!in_second && result.first.find(w) != result.first.end())) {
                    // we've forced the current vertex into the same bin as a neighbor: cannot be bipartite
                    result.first.clear();
                    result.second.clear();
    ...
    template<typename T, bool Weighted>
    std::list<std::pair<T, T>> maximum_bipartite_matching(const graph::graph<T, false, Weighted>& input, const T& dummy_1, const T& dummy_2) {
        std::vector<T> vertices = input.vertices();
        if(vertices.empty())
            return std::list<std::pair<T, T>>();
        if(std::find(vertices.begin(), vertices.end(), dummy_1) != vertices.end() || std::find(vertices.begin(), vertices.end(), dummy_2) != vertices.end())
        ...
        graph::graph<T, true, true> flow_graph(graph::adj_list);
        ...
        graph::graph<T, true, true> flow_result = Dinitz(flow_graph, dummy_1, dummy_2);
        std::list<std::pair<T, T>> result;
        for(const T& v : sets.first) {
            std::list<T> neighbors = flow_result.neighbors(v); // if non-empty, we found a flow to a neighbor
            if(neighbors.size() > 0)
