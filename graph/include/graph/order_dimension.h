...
namespace graph_alg {
    ...
    std::pair<std::list<Vertex>, std::list<Vertex>> two_dimensional_order_generator_closure(const graph::graph<Vertex, true, Weighted, Args...>&);
    ...
    std::pair<std::list<Vertex>, std::list<Vertex>> two_dimensional_order_generator(const graph::graph<Vertex, true, Weighted, EdgeWeight, Args...>& input) {
        ...
        // divide and conquer
        std::pair<std::list<Vertex>, std::list<Vertex>> front_order = two_dimensional_order_generator(input.generate_induced_subgraph(top_order.begin(), top_order.begin() + input.order() / 2));
        std::pair<std::list<Vertex>, std::list<Vertex>> back_order = two_dimensional_order_generator(input.generate_induced_subgraph(top_order.begin() + input.order() / 2, top_order.end()));
        ...
        graph::graph<Vertex, true, Weighted, EdgeWeight, Args...> matrix_rep = input.convert(graph::graph_type::adj_matrix);

        for (const Vertex& v : front_order.first) {
            // Forward sweep: find all (v, w) from (v,u) and (u, w) where u and w are in back_order
            auto it = std::find_if(back_order.first.begin(), back_order.first.end(), [&matrix_rep, &v](const Vertex& w) {return matrix_rep.has_edge(v, w);});
        ...
        for (const Vertex& v : back_order.first) {
            ...
            auto it = std::find_if(front_order.first.rbegin(), front_order.first.rend(), [&matrix_rep, &v](const Vertex& u) {return matrix_rep.has_edge(u, v);});
    ...
    graph::unweighted_graph<typename std::iterator_traits<It1>::value_type, true, Args...> three_dimensional_transitive_reduction(It1 first1, It1 last1, It2 first2, It2 last2, It3 first3, It3 last3) {
        if (std::distance(first1, last1) != std::distance(first2, last2) || std::distance(first2, last2) != std::distance(first3, last3))
...
Vertex temp;
        try {
        std::transform(first3, last3, isomorphism1.begin(), [&base_order, &temp](const Vertex& v) { temp = v; return base_order.at(v); }); 
        } catch (...) {
        ...
        try {
        std::transform(first3, last3, isomorphism2.begin(), [&base_order, &temp](const Vertex& v) { temp = v; return base_order.at(v); });
        } catch (...) {
        ...
        for (std::size_t i = 0; i < elements.size(); ++i) {
            ...
            for (std::size_t j = i + 1; j < elements.size(); ++j) {
                ...
                if (isomorphism1[i] < isomorphism1[j] && isomorphism2[i] < isomorphism2[j]) {
                    ...
                    // If there are any values where both isomorphisms give smaller numbers, there is a longer path around to current
                    auto pred = it;
