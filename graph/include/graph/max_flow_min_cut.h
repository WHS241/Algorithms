...
std::vector<std::list<std::pair<std::size_t, EdgeWeight>>> Ford_Fulkerson(
    std::vector<std::list<std::pair<std::size_t, EdgeWeight>>>& input, std::size_t source,
    std::size_t sink, Function f)
{
    ...
    try {
        while (true) {
            ...
            // verify path
            std::vector<EdgeWeight> net_flow(input.size(), zero);
...
graph::graph<Vertex, Directed, true, EdgeWeight, Args...> max_flow(
    const graph::graph<Vertex, Directed, true, EdgeWeight, Args...>& input, const Vertex& source,
    const Vertex& sink, Function flow_finder)
{
 ...
 */
template <typename EdgeWeight>
std::list<std::tuple<std::size_t, std::size_t, EdgeWeight>> Edmonds_Karp_helper(
    const exp_graph<EdgeWeight>& residual, std::size_t source, std::size_t sink)
{
...
graph::graph<Vertex, Directed, true, EdgeWeight, Args...> Edmonds_Karp_max_flow(
    const graph::graph<Vertex, Directed, true, EdgeWeight, Args...>& input, const Vertex& source,
    const Vertex& sink)
{
 ...
 */
template <typename EdgeWeight>
exp_graph<EdgeWeight> build_layer_graph(
    const exp_graph<EdgeWeight>& residual, std::size_t source, std::size_t sink)
{
 ...
 */
template <typename EdgeWeight>
std::list<std::tuple<std::size_t, std::size_t, EdgeWeight>> Dinic_helper(
    const exp_graph<EdgeWeight>& residual, std::size_t source, std::size_t sink)
{
    ...
    std::list<std::tuple<std::size_t,
        typename std::list<std::pair<std::size_t, EdgeWeight>>::iterator, EdgeWeight>>
        curr_path;

    std::list<std::tuple<std::size_t, std::size_t, EdgeWeight>> output;

    static EdgeWeight zero = EdgeWeight();
    static auto is_zero = [](const EdgeWeight& x) {
        if constexpr (std::is_floating_point_v<EdgeWeight>)
            return std::abs(x) <= 1e-10;
        else
            return x == zero;
    ...
    while (!curr_path.empty()) {
        ...
        if (!layer_graph[next_v].empty()) {
            ...
            // Add the path
            std::transform(curr_path.begin(), curr_path.end(), std::back_inserter(output),
                [&curr_flow](const std::tuple<std::size_t,
                    typename std::list<std::pair<std::size_t, EdgeWeight>>::iterator, EdgeWeight>&
                        step) {
...
template <typename Vertex, bool Directed, typename EdgeWeight, typename... Args>
graph::graph<Vertex, Directed, true, EdgeWeight, Args...> Dinic_max_flow(
    const graph::graph<Vertex, Directed, true, EdgeWeight, Args...>& input, const Vertex& source,
    const Vertex& sink)
{
...
std::list<cut_edge<Vertex>> minimum_cut(
    ...
    Function max_flow_alg = Edmonds_Karp_max_flow<Vertex, Directed, EdgeWeight, Args...>)
{
